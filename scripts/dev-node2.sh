...
NODE_NAME="meropixart-dev-2"
NODE_HOME="${MEROPIXART_DEV_NODE2_HOME:-$HOME/.calimero/meropixart-dev-2}"
...
pid_file() { echo "/tmp/meropixart-dev-node2.pid"; }
...
[ -f "$WASM_PATH" ] || { red "meropixart.wasm not found — run 'make dev-node' first"; exit 1; }
...
NODE1_LOG="/tmp/meropixart-dev-node.log"
NODE1_PEER_ID=""
if [ -f "$NODE1_LOG" ]; then
...
merod --node "$NODE_NAME" --home "$NODE_HOME" run \
  --auth-mode embedded \
  > "/tmp/meropixart-dev-node2.log" 2>&1 &
echo $! > "$(pid_file)"
green "Node2 started (pid $!  logs: /tmp/meropixart-dev-node2.log)"
wait_for_node
...
printf '  Logs:       /tmp/meropixart-dev-node2.log\n'
printf '\n'
printf '  Next step:\n'
